...
| `pickles/` | `Pickles` | the in-circuit kimchi verifier, so far its linearization slice: the `PolishToken` language and stack-machine interpreter, the reflection certificate identifying the deployed token stream with `gateLinearization`, the circuit reading proved to compute it, the `ft_eval0` gadget (`Pickles/FtEval0.lean`) proved to compute `ftEval0`, and the IPA-side scalar gadgets (`Pickles/{IPA,CombinedInnerProduct}.lean`: the challenge polynomial and the combined inner product, proved to read as the `Bulletproof` quantities). Requires snarky + kimchi + poseidon; own axiom gate, and the ONLY `native_decide` site in the tree outside CompElliptic and `Pasta/Endo.lean`. The token modules `Linearization/{Fp,Fq}.lean` are codegen output (`scripts/gen_tokens.lean`, via `make gen-linearization-lean`), committed because formal/'s CI checks out without the mina submodule |
