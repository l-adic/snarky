...
def rowHolds [CommRing F] (g : Gate F) (curr next : Row F) (pubr : F) : Prop :=
  ...
  -- The remaining custom gates are added one PR at a time ([04/13]…[07/13]); until then their
  -- rows are unconstrained here, so this slice's `check` is only meaningful on circuits built
  -- from Generic / Zero / CompleteAdd rows (e.g. the add_complete fixture).
  | .poseidon      => True
  | .varBaseMul    => True
  | .endoMul       => True
  | .endoMulScalar => True
...
theorem rowOk_iff [CommRing F] [DecidableEq F] (g : Gate F) (curr next : Row F) (pubr : F) :
  ...
  cases h : g.kind <;>
    simp [rowOk, rowHolds, h, AddComplete.ok_iff]
