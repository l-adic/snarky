...
`Kimchi/Pasta.lean` (whose import cone includes all of Mathlib via the EC oracle), while the
Fiat-Shamir layer (`Kimchi/Sponge`) consumes `vesta_lam` and `vesta_endo` from the fast
import lane.
...
the endomorphisms are the eigenvalue relations (`Kimchi.Pasta.pallas_eigen` /
`vesta_eigen`).
-/
...
/-- The scalar eigenvalue `λ` of the Pallas endomorphism `φ` — a primitive cube root of unity
    ...
    GLV short-basis fact in `Kimchi/Pasta.lean` is *proved*, not assumed. -/
def pallas_lam : ℤ :=
...
/-- The scalar eigenvalue `λ` of the Vesta endomorphism `φ` — a primitive cube root of unity
    ...
    `Kimchi/Pasta.lean` is proved; it is also the `λ` of the Fiat-Shamir challenge expansion
