...
> **Struck at iter-016 — 5 of the 8 are gone from the tree, so "all 8 preserved" is false.**
> Re-verified by unanchored, untruncated `grep -rc` over `*.lean` with `.lake` excluded:
> `verifierAcceptsAt_of_deferred_delta`, `honest_wins_everywhere`, `openingRelation_solvable`,
> `fiatShamirTreeB_trivial` and `kimchi_knowledge_soundness_conclusion_free_at_1dim` each return
> **zero** hits tree-wide. The three that remain live — `verifyWith_of_deferred_delta`,
> `honestNode_wins_everywhere`, `honestNode_wireWins_everywhere` — are the only rows below that
> still describe the tree. Per-row notes follow the table.
...
**Per-row status, re-verified at iter-016.** Dead: `verifierAcceptsAt_of_deferred_delta` — declaration deleted; the deferred-δ observation is not preserved anywhere. `honest_wins_everywhere` — declaration deleted; the argument is not preserved (see the prose-only list below, where it is also recorded as lost). `openingRelation_solvable` and `fiatShamirTreeB_trivial` — declarations deleted with `Forking/Triviality.lean`, which no longer exists. `kimchi_knowledge_soundness_conclusion_free_at_1dim` — declaration deleted with `Forking/Knowledge.lean`, exactly as step 6 contemplated. Live and unchanged in role: `verifyWith_of_deferred_delta` (`Forking/Deployed.lean`), `honestNode_wins_everywhere` and `honestNode_wireWins_everywhere` (`Forking/Honest.lean`, the latter also consumed by `Forking/KnowledgeSoundness.lean`).

~~**`fiatShamirTreeB_trivial` is not a historical note.**~~ **Superseded at iter-016 by the retirement of the Fiat–Shamir axioms.** `poseidon_fiat_shamir_{vesta,pallas}` no longer exist as declarations (declaration-level grep → 0; the five surviving textual hits are all historical notes in module docstrings and in `bulletproof-pcs/scripts/check_axioms.lean`), so the tautology-exhibit role this paragraph assigned to `fiatShamirTreeB_trivial` has no subject. `FiatShamirTreeB` itself is still a declaration (`Bulletproof/Soundness.lean:124`) and is still a hypothesis of the chunked headline; only the trivialization theorem is gone.

**Prose-only negative results that a file wipe destroys** (they have no declaration and exist nowhere else) — **status re-verified at iter-016**:
1. `Game.lean:1773-1801` — why the T/Pf-universally-quantified `honest_wins_everywhere` is **FALSE** (`Pf := Empty` makes `OracleComp T Pre Pf` empty; `Pf := Unit` fails because `sg` must vary with `u`). **Lost.** Neither the declaration nor the prose survives anywhere in the tree; a result this list designated never-to-lose was lost. Whether to restore it is a `bulletproof-pcs` judgement call and is deliberately not made here.
2. `EndoChallenge.lean:11-13` — that instantiating the game at `α := F` makes its counting hypothesis arithmetically unsatisfiable. Asserted in prose only; ideally becomes a decl. **Still live** — `EndoChallenge.lean` exists and has zero deletions under this plan.
3. `Model.lean:3-43` — the kimchi Poseidon-as-RO trust boundary and the cast/endoExpand idealization admission (step 2 re-sites it). **Superseded**: `Model.lean` is gone and the boundary is no longer prose at all — it is the hypothesis bundle `structure FSFaithful` (`kimchi/Kimchi/Verifier/Forking/Bridge.lean:93`), discharged per statement rather than trusted globally.

## E. Open gaps the plan does not close (record, don't bury)

> **Struck at iter-016 — items 1 to 4 no longer describe the tree; only item 5 is live.** Each
> strike below was re-verified with an unanchored, untruncated `grep -rc` over `*.lean` with
> `.lake` excluded, or by listing the directory the item names.

1. ~~**The 1-dim hypothesis is never discharged.**~~ **Superseded at iter-016.** `hspan` returns **zero** hits tree-wide and `Forking/Triviality.lean` no longer exists, so the "three vacuity theorems" this item was about are gone with it. The gap it named — a Pasta discharge of a 1-dimensionality premise — has no remaining statement to attach to.
2. ~~**kimchi still owes its own anti-vacuity companion.**~~ **Closed.** kimchi has two of its own: `honestKimchiFamily_wins` and `honestKimchiFamily_failure_set` (`kimchi/Kimchi/Verifier/Forking/Honest.lean`), both pinned by name in `kimchi/scripts/check_axioms.lean` and cited from the knowledge-soundness endpoints.
3. ~~**The kimchi prefix carrier admits no measure, and never did.**~~ **Superseded at iter-016**: the two files this item is about, `Escape.lean` and `GuardEscape.lean`, were deleted by this document's own step 1, and `kimchi/Kimchi/Verifier/Forking/` is now `Bridge`, `Honest`, `OracleRun`, `RunLink`, `Transcript`. There is no promised W4 lift left to be unperformable. The `frAbsorb : List C.ScalarField` retyping suggestion is the one part with content beyond the deleted files, and it is not acted on here.
4. ~~**Four factually wrong docstrings must not be carried forward.**~~ **Superseded at iter-016 — the three checkable ones are gone and the fourth is corrected.** `force-includes`, the import-cycle excuse and `foldAll` each return **zero** hits across `bulletproof-pcs/Bulletproof/Forking/*.lean`. `Deployed.lean`'s node-type paragraph now leads with the sound reason — the deployed prefix absorbs `δ` but not `sg`, so no function of the prefix list can return both `sg` values — and demotes finiteness to "a second benefit", which is what this item asked for. The `Adapter.lean` / `Convention.lean` clause is now **settled by a recorded negative check** (iter-019), which is as far as it can honestly be taken: the original text this clause paraphrased is not preserved anywhere in this document, so no positive match is possible and a strike on a reconstruction would be a guess. What was run, over both files as they stand: `grep -ic` for each of `force-include`, `import cycle`, `import-cycle`, `foldAll`, `live`, `obsolete`, `supersede`, `route` returns **0 in both** — every term the item itself names, plus the vocabulary any which-route-is-live claim would have to use. Both files were then read in full: `Adapter.lean` carries one preamble and five declaration docstrings, `Convention.lean` one preamble and seven, and every one of the fourteen is a factual statement of a structural correspondence (SRS ↔ URS field map, blinded ↔ de-blinded commitment, the `foldHalves`/`foldGens` inversion and its transport). `Convention.lean`'s `## Which convention is kimchi's` is about the *fold* convention, not a route, and its answer — kimchi's own, evidenced from `Wire.lean` and `Protocol.lean` — is correct. So: no claim of the described kind exists in either file today. The clause is retired as unmatchable, not as verified-wrong; if the original wording resurfaces, re-open it against that text rather than against this note.
5. **`roundNodeOf` (Deployed:586) is a verbatim copy of the record literal at :247-250 in the same file.** Define the branch once or the two will drift. **Still live** — `roundNodeOf` has 10 hits in `Forking/Deployed.lean` and nowhere else; the line numbers have drifted, the duplication has not been addressed.
