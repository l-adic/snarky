...
/-!
# The algebraic-prover corollary and the algebraic quotient, chunked

The chunked generalization of `Capstone/Algebraic.lean`, over the 44-row chunked
reduction (`Reduction/Chunked.lean`): the algebraic prover supplies SRS-basis
representations of every batch row's CHUNKS, one accepted opening of the chunked
combined claim discharges the consumer side, and the counting-SZ bad sets live at the
FLAT segment arity `∑ _ : Fin 44, nc` (`badXiOf`/`badROf` are arity-generic — the
flattening equiv `finSigmaFinEquiv` carries the chunk families to the flat batch the
opening actually combines, exactly as inside `chunked_batch_soundness`).

The algebraic quotient generalizes with genuine new content at `nc > 1`
(verifier.rs:960–965): the ft row's commitment collapses BOTH sides at
`ζ^{2^σ.k}` — the `f_comm` side is `pScalar` times the `nc`-chunk `σ₆` commitment
chunk-combined (at `nc = 1` this collapse was the identity), and the quotient side is
the `nt ≤ 7·nc`-chunk `t_comm` combination. `ftChunkAssembly` takes the chunk count as
a parameter; `ft_identity_of_chunks` derives the degree bound `< 7n` and the Maller
identity with the `σ₆`-side collapse resolved through per-chunk binding.
-/
...
theorem kimchiProof_sound_algebraic_ft [Field F] [AddCommGroup G]
  ...
  have hCσ6 : ∀ c : Fin nc,
    ...
    intro c
    have h : comms = Chunked.indexerOf σ nc idx := hvk
    rw [h]
    rfl
