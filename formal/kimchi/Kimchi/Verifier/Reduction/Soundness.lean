...
Recovered from the binding-free split `kimchiProof_sound_of_openings_of_vkrep`: `hbind`
discharges the two cross-point agreements (`bound_unique`) and, through
`chunk_rep_of_commit{,_masked}` — the trivial-relation half of
`dlRelation_of_chunk_rep{,_masked}_ne` — the four verifying-key row pinnings. -/
theorem kimchiProof_sound_of_openings [Field F] [AddCommGroup G] [Module F G]
  ...
  subst hvk'
  obtain ⟨hbounds, himp⟩ :=
    kimchiProof_sound_of_openings_of_vkrep σ idx hnc hk (indexerOf σ nc idx) pub wC zC pubC aw₀
  refine ⟨hbounds, ?_⟩
  intro β γ α t ζ E aw ρw hβ hγ hα hζ hζ₁ hζb ht hrow hteq
  refine himp β γ α t ζ E aw ρw hβ hγ hα hζ hζ₁ hζb ht hrow
    -- cross-point uniqueness per chunk: fixed commitments bind the reference chunks
    (fun col c => bound_unique σ hbind
    ...
    (fun c => bound_unique σ hbind
      ((hrow zRow c).1.trans ((hbound₀ zRow c).symm)))
    -- VK-row pinning: binding forces each verifying-key row's representation to be the
    -- honest chunk window of the circuit's own polynomial
    (fun i c => chunk_rep_of_commit σ hbind
      ((hrow (sRow i) c).1.trans
        (congrFun (batchC_sRow wC zC pubC (indexerOf σ nc idx) i) c)))
    (fun cc c => chunk_rep_of_commit σ hbind
      ((hrow (cRow cc) c).1.trans
        (congrFun (batchC_cRow wC zC pubC (indexerOf σ nc idx) cc) c)))
    (fun jj c => chunk_rep_of_commit_masked σ hbind
      ((hrow (selRow jj) c).1.trans
        ((congrFun (batchC_selRow wC zC pubC (indexerOf σ nc idx) jj) c).trans
          (congrFun (selComm_indexerOf σ nc idx jj) c))))
    (fun c => chunk_rep_of_commit_masked σ hbind
      ((hrow pubRow c).1.trans
