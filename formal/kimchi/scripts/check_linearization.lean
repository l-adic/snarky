  ...
  let r : Except String Bool := do
    ...
    let gates : List (String × F × F) :=
      [ ("generic", e.genericSelector
          * alphaCombo α ((Kimchi.Quotient.Gate.Generic.argument (F := F)).constraints gEnv),
          ← gateTarget "generic"),
        ("poseidon", e.poseidonSelector
          * alphaCombo α ((Kimchi.Quotient.Gate.Poseidon.argument (F := F)).constraints gEnv),
          ← gateTarget "poseidon"),
        ("completeAdd", e.completeAddSelector
          * alphaCombo α ((Kimchi.Quotient.Gate.AddComplete.argument (F := F)).constraints gEnv),
          ← gateTarget "completeAdd"),
        ("varBaseMul", e.mulSelector
          * alphaCombo α ((Kimchi.Quotient.Gate.VarBaseMul.argument (F := F)).constraints gEnv),
          ← gateTarget "varBaseMul"),
        ("endoMul", e.emulSelector
          * alphaCombo α ((Kimchi.Quotient.Gate.EndoMul.argument endo).constraints gEnv),
          ← gateTarget "endoMul"),
        ("endoScalar", e.endoScalarSelector
          * alphaCombo α ((Kimchi.Quotient.Gate.EndoScalar.argument (F := F)).constraints gEnv),
