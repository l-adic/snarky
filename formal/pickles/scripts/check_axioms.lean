...
`ft_eval0` circuit theorems built on them, and the two field-generic IPA-side gadget
theorems (the challenge polynomial and the combined inner product). Everything else the
