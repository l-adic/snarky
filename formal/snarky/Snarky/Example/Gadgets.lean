...
/-!
# The per-gadget D9 checks over the `Basic` backend

The regression suite behind `Snarky.Example`'s showcase: the fixed inputs the PS
QuickCheck specs sample (D9), as `decide` checks over `F17` — run the gadget under the
prover and compare its value against the spec's expectation. Everything reduces in the
kernel, so this file is the executable-semantics regression net the walk's gadget steps
left behind. Deliberately mechanical: one section per gadget family, in walk-step order,
opening with the smallest whole-stack circuit (witness, multiply, assert).
-/

namespace Snarky.Example

/-! ## The interpreter smoke test -/
