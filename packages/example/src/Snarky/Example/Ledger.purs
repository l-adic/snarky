...
import Snarky.Circuit.DSL (F, toField)
...
lookupAddress :: forall d f. Ord f => Ledger d f -> PublicKey (F f) -> Maybe (Sparse.Address d)
lookupAddress ledger pk = Map.lookup pk ledger.accountMap

-- | Fetch the account stored at an address (if any).
getAccount :: forall d f. Ledger d f -> Sparse.Address d -> Maybe (Account (F f))
...
balanceOf :: forall f. TokenAmount (F f) -> F f
