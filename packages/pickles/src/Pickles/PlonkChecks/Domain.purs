...
omegaPowers { generator, zkRows } = do
...
-- | `i ≤ maxLog2` by squaring, the entry at each domain's log2 selected by
-- | the which bits, minus one, sealed.
knownDomainVanishingPolynomial
  :: forall nd f r rd
   . PrimeField f
  => Reflectable nd Int
...
knownDomainVanishingPolynomial whiches domains maxLog2 zeta = do
