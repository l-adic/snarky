...
import Snarky.Circuit.Types (class CircuitType, FVar, Variable, fieldsToVar, sizeInFields, valueToFields, varToFields)
...
type SolverT f m a b =
  a -> ExceptT (EvaluationError f Variable) m (Tuple b (Map Variable f))

runSolverT
  :: forall f m a b
   . SolverT f m a b
  -> a
  -> m (Either (EvaluationError f Variable) (Tuple b (Map Variable f)))
...
runSolver
  :: forall f a b
   . Solver f a b
  -> a
  -> Either (EvaluationError f Variable) (Tuple b (Map Variable f))
...
makeChecker
  :: forall f c
   . (c -> Except (EvaluationError f Variable) Boolean)
  -> Checker f c
