#!/usr/bin/env bash
#
# Runs both sides of the Tree_proof_return base case with
# KIMCHI_WITNESS_DUMP enabled and diffs the 15-column kimchi witness
# matrices at every counter. This is the ground-truth correctness
# check — trace files are diagnostics; the witness fed into kimchi
# is definitional.
#
...
#   counter 2 → Tree_proof_return    step (Fp, VestaG, 67 public inputs)
#   counter 3 → Tree_proof_return    wrap (Fq, PallasG, 40 public inputs)
#
# Files:
#   /tmp/tree_oc_{0,1,2,3}.witness
#   /tmp/tree_ps_{0,1,2,3}.witness
#   /tmp/tree_witness_{nrr_step,nrr_wrap,tree_step,tree_wrap}.diff
#
# Exit code:
#   0   all four witness pairs byte-identical
#   1-4 bitfield of which pair diverged (+1 nrr_step, +2 nrr_wrap,
#       +4 tree_step, +8 tree_wrap)
#   >15 build/run failure
...
OC_TREE_STEP=/tmp/tree_oc_2.witness
OC_TREE_WRAP=/tmp/tree_oc_3.witness
PS_NRR_STEP=/tmp/tree_ps_0.witness
PS_NRR_WRAP=/tmp/tree_ps_1.witness
PS_TREE_STEP=/tmp/tree_ps_2.witness
PS_TREE_WRAP=/tmp/tree_ps_3.witness
...
for f in "$OC_NRR_STEP" "$OC_NRR_WRAP" "$OC_TREE_STEP" "$OC_TREE_WRAP"; do
  if [ ! -f "$f" ]; then
...
for label in "nrr_step:$OC_NRR_STEP" "nrr_wrap:$OC_NRR_WRAP" "tree_step:$OC_TREE_STEP" "tree_wrap:$OC_TREE_WRAP"; do
  name="${label%%:*}"
  file="${label#*:}"
...
for f in "$PS_NRR_STEP" "$PS_NRR_WRAP" "$PS_TREE_STEP" "$PS_TREE_WRAP"; do
  if [ ! -f "$f" ]; then
...
diff_pair tree_step 4 "$OC_TREE_STEP" "$PS_TREE_STEP" /tmp/tree_witness_tree_step.diff
diff_pair tree_wrap 8 "$OC_TREE_WRAP" "$PS_TREE_WRAP" /tmp/tree_witness_tree_wrap.diff
